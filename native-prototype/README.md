# YT Music Lite native Windows app

Browser-free Windows x64 music app with no WebView2 reference. Version 5.0.0 replaces the browser-based release.

## Run

The published package includes mpv, yt-dlp, and Deno beside `YTMusicLite.exe`. Source builds can also find those executables on PATH. Deno is used by yt-dlp's YouTube challenge solver.

Run `./build.ps1` in Windows PowerShell, then open `build/YTMusicLite.exe`. Use Search to find songs and artists, or Home → Import audio for local files. Double-click a track to play. The sidebar includes Home, Search, Library, Queue, Settings, and your playlists. Save search results to Library or a playlist; queue controls support previous/next and automatic advance. A native mini player provides transport controls. Library and playlists are saved under LocalAppData/YTMusicLiteNative, with a backup on replacement. Pause/resume controls mpv through a unique Windows named pipe. Stop and closing the app terminate its player and any active resolver subtree. The resolver exits after obtaining the audio URL. No browser profile or account cookies are accessed.
...
The Windows workflow runs `--ui-check` to exercise navigation, playlist display, queue removal, search isolation, the Settings updater, and mini-player creation, and exports normal/compact screenshots. Live search and streaming still require validation on a connected Windows machine.
